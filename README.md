...
> [!TIP]
> ### 🌐 想要免下載、立刻試玩看看？
> 如果您暫時不方便下載，可先前往 **[🤗 Hugging Face Space 線上 Demo 試玩](https://huggingface.co/spaces/Flow26/Gentleman-Mosaic-Next)** 體驗 AI 敏感部位偵測！  
> *(🛡️ 隱私承諾：線上版圖片僅於伺服器記憶體中暫態運算，運算完成即釋放，絕不留存任何照片)*
>
> ⚠️ **試玩須知（為什麼這只是 Demo？）**：  
> 單純依靠 AI 辨識，難免會遇到特殊角度**漏抓**或**誤判**。線上 Demo 僅用於基礎效果快測，**沒有提供畫布微調功能**，一旦 AI 漏抓將無法手動修補。  
>
> 若需要真正實用、滴水不漏的完整體驗，請使用本專案的**核心靈魂功能**（需下載 Next 懶人包）：
> - ⚡ **AI 批次 30 張打碼**：支援多圖一次匯入、自動排隊偵測、即時進度顯示。
> - 🎨 **互動畫布加減選修正**：本專案的核心靈魂！AI 框選後，隨時可用畫布筆刷塗抹補正，秒救漏抓、擦除誤判。
> - 🎛️ **樣式自由調整**：提供馬賽克尺寸自定義、海苔條紋自定義（顏色/寬度/間距/角度）、毛玻璃自定義。
> - 🔒 **100% 離線隱私保障**：226 MB 免安裝懶人包，解壓點擊即開，照片完全不出本機！
>
> 👉 **[點此前往 Releases 下載 v2.4.1 免安裝懶人包 (226 MB)](https://github.com/Chaos2-lgtm/Gentleman-Mosaic-Next/releases/tag/v2.4.1)**

### 📊 功能與體驗對照表

| 功能與體驗 | 🌐 線上試玩 Demo (HF Space) | 💻 本機旗艦版 (Next 懶人包) 🌟 推薦 |
| :--- | :---: | :---: |
| **使用門檻** | 免下載、瀏覽器直接開（手機/Mac 亦可） | 免裝 Python、226 MB 懶人包下載、解壓雙擊秒開 |
| **運算與隱私** | 雲端 ZeroGPU（純記憶體處理、不存檔） | **本機 100% 離線運算、照片絕不出個人電腦** |
| **處理張數** | 僅支援單張圖片上傳 | **支援最多 30 張批次處理 + 即時進度** |
| **AI 漏抓 / 誤判修正** | ❌ 無法修正（無畫布） | ✅ **核心靈魂！** 互動畫布框選 / 塗抹 / 加選 / 減選 |
| **打碼樣式自定義** | 僅基礎樣式 | **馬賽克尺寸自定義、海苔條紋自定義、毛玻璃自定義** |
| **歷史回溯** | ❌ 無法復原 | ✅ 完整支援 `Undo` / `Redo` 復原重做 |
